...
# ------------------------------------------------------------------------------
# 1. MODEL COEFFICIENT LOADER (Option 3: Reduced fallback weights)
# ------------------------------------------------------------------------------
if os.path.exists(MODEL_JSON_PATH):
    ...
    COEFS = {
        ...
        "hypertension": 0.85,
        "smoke": 0.75,
        "fam_stroke": 0.12,          # Option 3: Calibrated down from 0.38 to 0.12
        "fam_smoke": 0.10,           # Option 3: Calibrated down from 0.25 to 0.10
        "aqi_scaled": 0.14,
        "aqi_x_heart": 0.08,
        "aqi_x_hypertension": 0.07,
        "aqi_x_age": 0.04,
    }
...
def geocode_address(query: str) -> tuple[float, float, str]:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "AirBuddyHealth/11.0 (community-check)"})
    with urllib.request.urlopen(req, timeout=10) as resp:
        data = json.loads(resp.read().decode("utf-8"))

    if not data:
        fallback_url = f"https://nominatim.openstreetmap.org/search?q={encoded}&format=json&limit=1"
        req_fallback = urllib.request.Request(fallback_url, headers={"User-Agent": "AirBuddyHealth/11.0 (community-check)"})
...
def fetch_purpleair_aqi(lat: float, lon: float) -> tuple[float, str, str, float, float]:
    if PURPLEAIR_API_READ_KEY == "YOUR_PURPLEAIR_READ_KEY_HERE":
        raise ValueError("PurpleAir API key is missing.")
...
def compute_detailed_risk(
    age: int, hypertension: bool, heart: bool, smoke: bool, fam_smoke: bool, fam_stroke: bool, aqi: float
    ...
    age_contrib = COEFS.get("age_over_40", 0.65) * age_over_40
    heart_contrib = COEFS.get("heart", 1.35) * h_val + COEFS.get("aqi_x_heart", 0.08) * (aqi_scaled * h_val)
    ht_contrib = COEFS.get("hypertension", 0.85) * ht_val + COEFS.get("aqi_x_hypertension", 0.07) * (aqi_scaled * ht_val)
    smoke_contrib = COEFS.get("smoke", 0.75) * s_val
    stroke_contrib = COEFS.get("fam_stroke", 0.12) * f_stroke_val
    smoke_fam_contrib = COEFS.get("fam_smoke", 0.10) * f_smoke_val
    aqi_contrib = COEFS.get("aqi_scaled", 0.14) * aqi_scaled + COEFS.get("aqi_x_age", 0.04) * (aqi_scaled * age_over_40)
