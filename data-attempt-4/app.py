...
# ------------------------------------------------------------------------------
# 1. MODEL COEFFICIENT LOADER (Calibrated & Tempered)
# ------------------------------------------------------------------------------
if os.path.exists(MODEL_JSON_PATH):
    with open(MODEL_JSON_PATH, "r", encoding="utf-8") as f:
        _loaded = json.load(f)
        INTERCEPT = _loaded.get("intercept", -4.70)
        COEFS = _loaded.get("coefficients", {})
else:
    # Calibrated fallback: Baseline reduced to -4.70, feature impacts reduced
    INTERCEPT = -4.70
    COEFS = {
        "age_over_40": 0.27,
        "heart": 0.16,
...
# Calibrated classification thresholds aligned with the lowered baseline
LEVEL_THRESHOLDS = [
    (0.25, 5, "#ef4444", "Take Immediate Shelter", "🚨 Red Flag Alert", "Your body is under major strain from the air quality outside. Close all windows, turn on an air filter if you have one, and rest indoors. If you feel dizzy, short of breath, or notice sudden numbness, contact a doctor or dial 911 right away."),
    (0.14, 4, "#f97316", "Stay Indoors & Rest", "⚠️ High Strain Zone", "Skip outdoor exercise or heavy chores today. Keep inside with clean air, drink extra water, and make sure your regular prescriptions or inhalers are within reach."),
    (0.07, 3, "#f59e0b", "Pace Yourself", "👀 Yellow Caution", "If you have heart or blood pressure concerns, take things slow outside. Avoid long walks along busy roads while the air is hazy or dusty."),
    (0.03, 2, "#10b981", "Looking Safe", "👍 Green Light", "Your body and the outdoor air are in good harmony today. You are free to run errands, play, and go on walks."),
...
def pm25_to_aqi(pm: float) -> float:
    # Continuous EPA breakpoints without floating-point fall-through gaps
    breakpoints = [
        (0.0, 9.0, 0.0, 50.0),
        (9.0, 35.4, 51.0, 100.0),
...
# ------------------------------------------------------------------------------
# 3. INTERACTIVE LOGISTIC REGRESSION SCORING (Tempered)
# ------------------------------------------------------------------------------
def compute_detailed_risk(
    ...
    logit = (
        INTERCEPT
        + COEFS.get("age_over_40", 0.27) * age_over_40
        + COEFS.get("heart", 0.16) * h_val
        + COEFS.get("hypertension", 0.16) * ht_val
        + COEFS.get("smoke", 0.10) * s_val
        + COEFS.get("fam_stroke", 0.12) * f_stroke_val
        + COEFS.get("fam_smoke", 0.07) * f_smoke_val
        + COEFS.get("aqi_scaled", 0.015) * aqi_scaled
        + COEFS.get("pm2_5_scaled", 0.043) * pm2_5_scaled
        + COEFS.get("o3_scaled", 0.064) * o3_scaled
        + COEFS.get("no2_scaled", 0.037) * no2_scaled
    )
...
<div class="card">
  ...
  <div id="result">
    ...
    <div class="explainer-card">
      ...
      <p class="explainer-p">
        <strong>1. We check outdoor sensors live:</strong> We pull readings from local ground monitors for particles and gases right outside your window.
      </p>
      <p class="explainer-p">
        <strong>2. We connect your health background:</strong> Tiny particles and exhaust gases enter airways and blood vessels. If you already have higher blood pressure or cardiac strain, dirty air causes a larger relative strain spike.
      </p>
      <p class="explainer-p">
...
function displayResults(data) {
  ...
  // Thermometer Pin mapped to 0.0 - 0.25 probability scale
  const pin = document.getElementById("meter-pin");
  const pinPos = Math.min(96, Math.max(4, (data.probability / 0.25) * 100));
